...
| PostgreSQL | **Neon** | serverless Postgres, generous free row/storage caps |
| Qdrant | **Qdrant Cloud** | 1 GB free is far more than a 40-item catalog needs |
| Redis | **Upstash** | free tier; used only for the L2 cache and the regeneration lock |
...
1. **Provision Neon, Upstash and Qdrant Cloud** and copy their connection strings — this is the only
   manual infra step; nothing else is provisioned automatically.
   - Neon → Postgres connection string, rewritten to the `postgresql+asyncpg://` driver with
     `?sslmode=require` appended → `DATABASE_URL`.
   - Upstash → the Redis URL (`redis://default:<password>@<host>:<port>`) → `REDIS_URL`.
...
2. **Deploy the backend to Render**: dashboard → New → Blueprint → point at this repo. Render reads
   [`render.yaml`](render.yaml), which builds [`Dockerfile`](Dockerfile) as a Docker web service with
   `healthCheckPath: /health`. Fill in the `sync: false` env vars in the dashboard (table below);
   `JWT_SECRET` and `DIGEST_TRIGGER_TOKEN` are auto-generated by the blueprint. **Migrations run
   automatically on every boot** — [`docker-entrypoint.sh`](docker-entrypoint.sh) runs
   `alembic upgrade head` before starting uvicorn, so there is no separate release-command step to
   configure.
